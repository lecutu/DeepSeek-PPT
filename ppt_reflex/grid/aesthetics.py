"""
grid/aesthetics.py — 美观性规则引擎（防丑下限）
只判量化违规，不判审美优劣。
所有规则满足: 可量化/可检测/有依据(WCAG 2.1 / 排版学)。
"""
...
class AestheticsEngine:
    ...
    def _color(self, e):
        ...
        if ct < mn:
            ...
            v.append(self._violation("color_contrast", "color", "P0", Verdict.BLOCK, e.id,
                f"对比度 {ct:.1f}:1 < {mn}:1 ({lbl})", {"ratio": round(ct,1), "req": mn}))
        if e.font_color == e.fill_color and e.font_color not in ("FFFFFF","000000","","transparent","none"):
            v.append(self._violation("invisible_text", "color", "P0", Verdict.BLOCK, e.id,
                f"文字=填充色({e.font_color})"))
        Lb = luminance_L(fillc); Lt = luminance_L(fc)
        if Lb < 50 and Lt < 128:
            v.append(self._violation("dark_bg_dark_text", "color", "P0", Verdict.BLOCK, e.id,
                f"深底(L*={Lb:.0f})配深字(L*={Lt:.0f})", {"bg_L": round(Lb), "tx_L": round(Lt)}))
        if Lb > 85 and Lt > 128:
            v.append(self._violation("light_bg_light_text", "color", "P0", Verdict.BLOCK, e.id,
                f"浅底(L*={Lb:.0f})配浅字(L*={Lt:.0f})", {"bg_L": round(Lb), "tx_L": round(Lt)}))
        return v
    ...
    def _style(self, e, all_elements):
        ...
        try:
            ...
            if Lb < 8 and e.content_type in (ContentType.BACKGROUND, ContentType.TEXTBOX):
                v.append(self._violation("no_black_bg", "style", "P0", Verdict.BLOCK, e.id,
                    f"纯黑底(L*={Lb:.0f})", {"L": round(Lb)}))
    ...
    def _font(self, e):
        ...
        if e.font_size < 10:
            v.append(self._violation("font_abs_min", "font", "P0", Verdict.BLOCK, e.id,
                f"字号{e.font_size}pt<10pt", {"size": e.font_size}))
        if 10 <= e.font_size < 14 and e.content_type != ContentType.FOOTER:
            v.append(self._violation("font_rec_min", "font", "P1", Verdict.WARN, e.id,
                f"字号{e.font_size}pt<14pt推荐", {"size": e.font_size}))
            ...
            if (ox > 2 or oy > 2) and e.h > 0:
                ...
                if fitted < 10:
                    v.append(self._violation("autofit_tiny", "font", "P0", Verdict.BLOCK, e.id,
                        f"自动缩放后{fitted:.0f}pt<10pt", {"fitted": round(fitted,1)}))
        return v
        ...
        if e.auto_size == "NONE":
            if not e.word_wrap and ox > 2:
                v.append(self._violation("overflow_h", "overflow", "P0", Verdict.BLOCK, e.id,
                    f"水平溢出{ox:.0f}pt(未换行)", {"ox": round(ox,1)}))
            if oy > 2:
                v.append(self._violation("overflow_v", "overflow", "P0", Verdict.BLOCK, e.id,
                    f"垂直溢出{oy:.0f}pt", {"oy": round(oy,1)}))
            fixes = []
            fitted = e.font_size * min(e.w/max(rw,0.1), e.h/max(rh,0.1))
            if fitted >= 10: fixes.append(f"开自动缩放→字号~{fitted:.0f}pt")
            if not e.word_wrap and ox > 2: fixes.append("开自动换行")
            fixes.append("扩框适配内容")
            fixes.append("删减文字或拆到下一页")
            ...
            v.append(self._violation("autofit_expand", "overflow", "P1", Verdict.WARN, e.id,
                f"框将扩张{ox:.0f}x{oy:.0f}pt", {"ox": round(ox,1), "oy": round(oy,1)}))
        return v
    ...
    def _align(self, elements):
        ...
        for e in non_locked:
            ...
            v.append(self._violation("grid_offset", "alignment", "P2", Verdict.WARN, e.id,
                f"距网格{off:.0f}pt", {"offset": round(off,1)}))
        return v
