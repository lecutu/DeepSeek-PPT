...
def _render_image(slide, x: float, y: float, w: float, h: float,
    ...
    """等比缩放(contain-fit)渲染图片。绝不拉伸变形，绝不裁剪溢出。"""
    import os
    from pptx.util import Pt, Emu
    ...
    box_aspect = max_w_pt / max_h_pt

    # ── fit_mode: fill (兼容旧roadmap) / crop_center (允许裁切, 用于封面) / fit(default-等比contain) ──
    if p.fit_mode == "fill":
        pic = slide.shapes.add_picture(p.image_path, Pt(x), Pt(y), Pt(max_w_pt), Pt(max_h_pt))

    elif p.fit_mode == "crop_center":
        if img_aspect > box_aspect:
            scale = max_h_pt / img_h_px
            scaled_w = img_w_px * scale
            crop_x = x - (scaled_w - max_w_pt) / 2
            pic = slide.shapes.add_picture(p.image_path, Pt(crop_x), Pt(y), Pt(scaled_w), Pt(max_h_pt))
        else:
            scale = max_w_pt / img_w_px
            scaled_h = img_h_px * scale
            crop_y = y - (scaled_h - max_h_pt) / 2
            pic = slide.shapes.add_picture(p.image_path, Pt(x), Pt(crop_y), Pt(max_w_pt), Pt(scaled_h))

    else:  # "fit" — contain-fit: 等比缩放到完全落在区域内部, 不裁剪
        # Step 1: contain-fit 计算 final_w, final_h (≤ max_w, max_h)
        if img_aspect > box_aspect:
            # 图比框更宽 → 受框宽约束
            fit_w = max_w_pt
            fit_h = max_w_pt / img_aspect
        else:
            # 图比框更高 → 受框高约束
            fit_h = max_h_pt
            fit_w = max_h_pt * img_aspect

        # Step 2: no-upscale guard — 绝不放大小图
        if not getattr(p, 'allow_upscale', False):
            scale_x = fit_w / img_w_px
            scale_y = fit_h / img_h_px
            # 若需要放大才算满，则保持原始尺寸 1:1
            if scale_x > 1.0:
                fit_w = float(img_w_px)
                fit_h = float(img_h_px)
            elif scale_y > 1.0:
                fit_w = float(img_w_px)
                fit_h = float(img_h_px)
            # 注意：如果原始图本身就大于框，等比例缩小是 OK 的(scale < 1.0)

        # Step 3: min-size guard (≥40x30pt)
        MIN_W, MIN_H = 40.0, 30.0
        if fit_w < MIN_W or fit_h < MIN_H:
            print(f"[serializer] IMAGE_TOO_SMALL: {p.image_path} → {fit_w:.0f}x{fit_h:.0f}pt < {MIN_W:.0f}x{MIN_H:.0f}pt")

        # Step 4: invariant check (aspect ratio must be preserved)
        final_aspect = fit_w / fit_h if fit_h > 0 else img_aspect
        if abs(final_aspect - img_aspect) > 0.001:
            print(f"[serializer] ASPECT_RATIO_BROKEN: {p.image_path} aspect={img_aspect:.4f} final={final_aspect:.4f}")

        # Step 5: 居中放置
        fit_x = x + (max_w_pt - fit_w) / 2
        fit_y = y + (max_h_pt - fit_h) / 2
